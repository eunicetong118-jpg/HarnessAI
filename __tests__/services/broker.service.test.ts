...
describe('Broker Service', () => {
  ...
  describe('linkAccount', () => {
    it('creates a BrokerAccount and a VERIFICATION ticket on success', async () => {
      ...
      (prisma.brokerAccount.create as any).mockResolvedValue({ id: 'ba-1', userId, mt5AccountNo });
      (prisma.ticket.create as any).mockResolvedValue({ id: 't-1', userId, type: 'VERIFICATION' });
    ...
    it('throws error if MT5 account is already linked', async () => {
      (prisma.brokerAccount.findUnique as any).mockResolvedValue({ id: 'existing' });
  ...
  describe('hasLinkedAccount', () => {
    it('returns true if user has at least one broker account', async () => {
      (prisma.brokerAccount.count as any).mockResolvedValue(1);
    ...
    it('returns false if user has no broker accounts', async () => {
      (prisma.brokerAccount.count as any).mockResolvedValue(0);
      const result = await hasLinkedAccount(userId);
