...
classMapper
   method Map(id n, object N)
      Emit(id n, object N)
      forall id m in N.OutgoingRelations do
...
[![Iterative Message Passing](https://highlyscalable.wordpress.com/wp-content/uploads/2012/01/graph-propagation-3.png "Iterative Message Passing")](https://highlyscalable.wordpress.com/wp-content/uploads/2012/01/graph-propagation-3.png)
...
```cpp
classN
   State in {True = 2, False = 1, null = 0}, initialized 1 or 2 forend-of-line categories, 0 otherwise
...
```cpp
classN
   State is distance, initialized 0 forsource node, INFINITY forall other nodes
...
method getMessage(object N)
    returnN.State / N.OutgoingRelations.size()
...
classReducer
   method Initialize
      H = newAssociativeArray : category -> count
...
classMapper
   method Map(null, items [i1, i2,...] )
      forall item i in [i1, i2,...]
         H = newAssociativeArray : item -> counter
         forall item j in [i1, i2,...]
            H{j} = H{j} + 1
...
classReducer
   method Reduce(join_key k, tagged_tuples [t1, t2,...])
      H = newAssociativeArray : set_name -> values
...
classMapper
   method Initialize
      H = newAssociativeArray : join_key -> tuple from R
      R = loadR()
...
- C. T. Chu *et al* provides an excellent description of machine learning algorithms for MapReduce in the article [Map-Reduce for Machine Learning on Multicore](http://www.cs.stanford.edu/people/ang//papers/nips06-mapreducemulticore.pdf "Map-Reduce for Machine Learning on Multicore").
- FFT using MapReduce: [http://www.slideshare.net/hortonworks/large-scale-math-with-hadoop-mapreduce](http://www.slideshare.net/hortonworks/large-scale-math-with-hadoop-mapreduce)
- MapReduce for integer factorization: [http://www.javiertordable.com/files/MapreduceForIntegerFactorization.pdf](http://www.javiertordable.com/files/MapreduceForIntegerFactorization.pdf)
- Matrix multiplication with MapReduce: [http://csl.skku.edu/papers/CS-TR-2010-330.pdf](http://csl.skku.edu/papers/CS-TR-2010-330.pdf) and [http://www.norstad.org/matrix-multiply/index.html](http://www.norstad.org/matrix-multiply/index.html)